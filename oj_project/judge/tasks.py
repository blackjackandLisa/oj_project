...
def judge_submission(submission_id):
    ...
    # 记录：开始判题
    log_submission_event(submission, 'judging', {'start_time': time.time()})
    ...
    try:
        ...
        if not is_safe:
            ...
            log_submission_event(submission, 'security_check_failed', {'reason': error_msg})
        ...
        # 记录：系统错误
        log_submission_event(submission, 'error', {'exception': str(e)})
        ...
        try:
            ...
            try:
                ...
                process = subprocess.run(
                    ['/usr/local/bin/python', code_file],  # 使用完整路径
                    input=test_case.input_data,
                    capture_output=True,
                    text=True,
                    timeout=problem.time_limit / 1000.0,
                    check=False,
                    env={'PATH': '/usr/local/bin:/usr/bin:/bin'},  # 保留最小PATH
                    cwd='/tmp',  # 设置工作目录
